...
function makeUI()
{
    ...
    italic.appendChild(document.createTextNode('انقر فوق اسم المستخدم لإنشاء حساب وهمي أوفلاين عن طريقة كتابة ID الحساب المكون من 16 رقم'));
    ...
    for(var i = 0; i < 16; i++)
    {
        ...
        button.appendChild(document.createTextNode('تفعيل و تنشيط الحساب'));
...
function fetchAccount(i)
{
    ...
    if(!mailbox.account_name)
    {
        ...
        italic.appendChild(document.createTextNode('حساب غير موجود'));
        ui[i].id.value = '';
...
function activateAccount(idx)
{
    ...
    for(var i = 7; i >= 0; i--)
    {
        ...
        if(digit != digit)
        {
            alert("يجب أن يكون طوله 16 حرفًا وأن يحتوي فقط على [0-9A-F]");
            return;
        }
    ...
    if(ui[idx].id.value == acc_id.toUpperCase())
        alert("تم تسجيل حساب PSN وهمي أوفلاين بنجاح!");
    return false;
}
